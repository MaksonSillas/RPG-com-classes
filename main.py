class Villager:
    ...
    def check_health(self, incoming_attack_value: int):
        if not self.defense >= incoming_attack_value:
            ...
            return self.health
        return "The target's defense is greater than the received attack"

    def normal_attack(self, target: object):
        attack = self.attack - target.defense
...
class Mage(Villager):
    ...
    def fire_ball(self, target: object):
        if self.mana >= 20:
            self.mana -= 20
            attack = self.attack + 20 - target.defense
